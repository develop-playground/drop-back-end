...
public class MemoryController {
    ...
    public ResponseEntity<MemoryResponseDto.Find> findMemory(
        @Parameter(description = "조회할 추억 ID") @PathVariable Long id
    ) {
    ...
    public ResponseEntity<List<MemoryResponseDto.Find>> findAllMemory(
        @Schema(hidden = true) @RequestHeader("Authorization") String authorization,
        @Schema(hidden = true) @PageableDefault(size=10, sort="id", direction = Sort.Direction.DESC) Pageable pageable
    ) {
    ...
    public ResponseEntity<MemoryResponseDto.Update> changeContent(
        @Schema(hidden = true) @RequestHeader("Authorization") String authorization,
        @Parameter(description = "수정할 추억 ID") @PathVariable final Long id,
    ...
    public ResponseEntity<String> deleteMemory(
        @Schema(hidden = true) @RequestHeader("Authorization") String authorization,
        @Parameter(description = "삭제할 추억 ID") @PathVariable final Long id
    ) {
